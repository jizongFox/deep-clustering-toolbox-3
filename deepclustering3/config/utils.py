...
from deepclustering3.utils.types import mapType
...
def extract_subdictionary_from_large(large_dictionary, small_dictionary, deepcopy=True):
    if deepcopy:
    ...
    for k, v in small_dictionary.items():
        if k in large_dictionary:
            if not isinstance(v, mapType):
                ...
                small_dictionary[k] = extract_subdictionary_from_large(large_dictionary[k], small_dictionary[k])

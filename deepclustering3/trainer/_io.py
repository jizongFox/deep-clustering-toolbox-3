...
from ..meters.storage_interface import Storage
from ..types import typePath
from ..utils.io import path2Path, yaml_write
...
class _IOMixin(_BufferMixin, metaclass=ABCMeta):
    ...
    def dump_config(self, path: typePath = None, save_name="config.yaml"):
        if path:
            path_ = path2Path(path)
            if not path_.is_absolute():
                path_ = Path(self.RUN_PATH) / path_
            yaml_write(self._config, str(path_), save_name=save_name)
